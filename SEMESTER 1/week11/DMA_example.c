...
This program uses malloc() to dynamically allocate a block of memory, enter data into the memory block and display it. The memory block is freed
(released) at the end when the memory block is no longer required. 
*/ 
...
{ 
    ...
    int numbers = 0; 
 	...
 	// Part 4 
 	// Check if malloc was successful, i.e., check if the memory was dynamically allocated successfully 
 	if (ptr == NULL) 
 	{ 
